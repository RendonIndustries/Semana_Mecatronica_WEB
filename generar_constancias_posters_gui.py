...
def agregar_qr_y_folio(slide, qr_data: str, folio: str, slide_width, slide_height) -> None:
    ...
    if placeholder is not None:
        ...
        text_height = Cm(0.6)
        gap = Cm(0.1)
        margin = int(min(placeholder.width, placeholder.height) * 0.05)
        available_height = placeholder.height - text_height - gap - (margin * 2)

        if available_height > 0:
            qr_size = min(placeholder.width - (margin * 2), available_height)
            show_folio = True
        else:
            qr_size = min(placeholder.width - (margin * 2), placeholder.height - (margin * 2))
            show_folio = False

        qr_left = placeholder.left + (placeholder.width - qr_size) / 2
        qr_top = placeholder.top + margin

        if show_folio:
            qr_top = placeholder.top + margin
            folio_top = qr_top + qr_size + gap
        else:
            qr_top = placeholder.top + (placeholder.height - qr_size) / 2
            folio_top = None

        slide.shapes.add_picture(qr_stream, qr_left, qr_top, width=qr_size, height=qr_size)

        if show_folio and folio_top is not None:
            textbox = slide.shapes.add_textbox(qr_left, folio_top, qr_size, text_height)
    ...
    textbox_height = Cm(0.6)
    gap = Cm(0.1)
    left = margin
    top = slide_height - qr_size - margin - textbox_height - gap

    slide.shapes.add_picture(qr_stream, left, top, width=qr_size, height=qr_size)

    textbox = slide.shapes.add_textbox(left, top + qr_size + gap, qr_size, textbox_height)
    p = textbox.text_frame.paragraphs[0]
    p.text = f"FOLIO: {folio}"
    p.font.size = Pt(8)
    p.alignment = PP_ALIGN.CENTER
...
class App(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        self._row(frame, "Carpeta de salida:", self.output_var, self._browse_output, 2)
    ...
    def _row(self, parent, label, variable, command, row_idx: int) -> None:
        row = tk.Frame(parent)
